# NOTICE

## Argo for Office 365 Excel

Copyright © 2016-2025 Booz Allen Hamilton

This project contains software developed by Booz Allen Hamilton (http://www.boozallen.com/).

---

## Third-Party Software Notices and Licenses

This software includes or depends upon the following third-party components:

### Runtime Dependencies

#### React (MIT License)
Copyright (c) Meta Platforms, Inc. and affiliates.
https://reactjs.org/
License: MIT

#### TypeScript (Apache 2.0 License)
Copyright (c) Microsoft Corporation.
https://www.typescriptlang.org/
License: Apache 2.0

#### Office.js
Copyright (c) Microsoft Corporation.
https://learn.microsoft.com/javascript/api/office
License: Microsoft Software License Terms

#### math.js (Apache 2.0 License)
Copyright (c) 2013-2025 Jos de Jong
https://mathjs.org/
License: Apache 2.0

#### Recharts (MIT License)
Copyright (c) 2015-2025 Recharts Group
https://recharts.org/
License: MIT

#### Fluent UI React (MIT License)
Copyright (c) Microsoft Corporation
https://developer.microsoft.com/fluentui
License: MIT

### Development Dependencies

#### Vite (MIT License)
Copyright (c) 2019-present Evan You & Vite Contributors
https://vitejs.dev/
License: MIT

#### Jest (MIT License)
Copyright (c) Meta Platforms, Inc. and affiliates.
https://jestjs.io/
License: MIT

#### Playwright (Apache 2.0 License)
Copyright (c) Microsoft Corporation
https://playwright.dev/
License: Apache 2.0

#### ESLint (MIT License)
Copyright (c) OpenJS Foundation and ESLint contributors
https://eslint.org/
License: MIT

#### Prettier (MIT License)
Copyright (c) James Long and contributors
https://prettier.io/
License: MIT

---

## Original Argo (v1.x) Dependencies

The original Argo project for Excel 2007-2016 used the following dependencies:

#### Excel-DNA (MIT License)
Copyright (c) Govert van Drimmelen
https://excel-dna.net/
License: MIT

#### Math.NET Numerics (MIT License)
Copyright (c) Math.NET
https://numerics.mathdotnet.com/
License: MIT

#### Extended WPF Toolkit (MS-PL)
Copyright (c) Xceed Software Inc.
https://github.com/xceedsoftware/wpftoolkit
License: Microsoft Public License (MS-PL)

---

## Reverse Engineering Notice

This project (Argo v5.0) is a **reverse engineering effort using Claude Code (AI)** to recreate the functionality of the original Argo based on available documentation. The original source code for Argo (v1.x through v4.x for Excel 2007-2016) was lost.

The implementation in this repository is a clean-room rebuild that:
- Does not include any original source code
- Was developed based solely on public documentation, user guides, and feature descriptions
- Uses entirely different technology stack (TypeScript/React/Office.js vs .NET/VSTO)
- Maintains compatibility with the original Argo's licensing approach (Apache 2.0)

---

## License

This project is licensed under the Apache License 2.0. See [LICENSE.md](LICENSE.md) for the full license text.

---

## Attributions

### Original Argo Team
- Developed by Booz Allen Hamilton
- Released as open source in 2016
- No longer under active development as of 2022

### Argo v5.0 Team
- Reverse engineered with Claude Code (AI) assistance
- Based on documentation from original project
- Targeting modern Office 365 Excel platform

---

## Export Control

This software may be subject to U.S. export control laws. Users are responsible for ensuring compliance with all applicable export regulations.

---

For complete licensing information, see the LICENSE.md file in this repository.
